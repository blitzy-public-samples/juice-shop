...
// [SECURITY FIX] Broken Authentication
// Issue: jsonwebtoken@0.4.0 derives the token solely from the payload plus a second-granularity `iat`, with no random component, so authorize() returned a BYTE-IDENTICAL JWT for the same user issued within the same second. Combined with the whole-token logout denylist, an immediate re-login right after logout handed back the just-denylisted token, so protected routes rejected it (401) and the user was locked out until the next second (QA Issue #2 / SC5 edge case).
// Risk: CWE-613 (session management) — a legitimate re-login could return an already-invalidated token; more generally, non-unique tokens make a whole-token denylist ambiguous across distinct sessions of the same user.
// Fix: Add a unique per-issue `jti` nonce (128-bit cryptographically random) to every signed token so each issued JWT is unique regardless of payload or timing. A fresh login therefore always yields a token distinct from any previously denylisted one. The nonce is added to a SHALLOW COPY of the payload (object spread) so the caller's object (the one stored in authenticatedUsers) is never mutated; the pinned RS256 algorithm and the 1-hour exp are left unchanged.
export const authorize = (user = {}) => jwt.sign({ ...user, jti: crypto.randomBytes(16).toString('hex') }, privateKey, { expiresInMinutes: 60, algorithm: 'RS256' } as any)
// [SECURITY FIX] Broken Authentication
